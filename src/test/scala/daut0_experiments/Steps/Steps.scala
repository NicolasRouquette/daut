...
abstract class Step(var stepName: String, var dtStable: Int, timeout: Int) extends Monitor[IO, daut0_experiments.Events.Event]:
  ...
  def isInOneOfStates(names: String*): Boolean =
    exists { case fact => names.contains(fact.getClass.getSimpleName) }
  ...
  def isSuccessful(untilState: Option[String], untilStep: Option[Step]): Boolean =
    (untilState, untilStep) match
      ...
      case _ => assert(false).asInstanceOf[Boolean]
...
abstract class Observe(stepName: String, dtStable: Int, timeout: Int) extends Step(stepName, dtStable, timeout):
  ...
  case class Active[S](
    ...
    watch:
      case Value[S @unchecked](t_NEW, eventName, x_NEW) if eventName == observedEventName =>
        if !isInRange(x) && isInRange(x_NEW) then
          IO.pure(Set(Active(t_start, t_NEW, x_NEW, isInRange, untilStep, untilState, observedEventName)))
        else
          IO.pure(Set(Active(t_start, t, x_NEW, isInRange, untilStep, untilState, observedEventName)))
      case Time(t_NEW) =>
        if isTimedout(t_NEW, t_start) then
          IO.pure(Set(Failed(t_NEW, s"timeout at t=$t_NEW")))
        else if isInRange(x) && isStable(t_NEW, t) then
          IO.pure(Set(Done[S](t_NEW, x, isInRange, untilStep, untilState, observedEventName)))
        else
          IO.pure(Set(Active(t_start, t, x, isInRange, untilStep, untilState, observedEventName)))
  ...
  case class Done[S](
    ...
    watch:
      case Value[S @unchecked](t_NEW, eventName, x_NEW) if eventName == observedEventName =>
        if isSuccessful(untilState, untilStep) then
          IO.pure(Set(Success(t_NEW)))
        else if t_NEW < t then
          IO.pure(Set(Done(t, x, isInRange, untilStep, untilState, observedEventName)))
        else if !isInRange(x_NEW) then
          IO.pure(Set(Failed(t_NEW, s"altered value at t=$t_NEW")))
        else
          IO.pure(Set(Done(t_NEW, x_NEW, isInRange, untilStep, untilState, observedEventName)))

  case class Failed(t: Int, msg: String) extends fact:
    watch:
      case _ => IO.pure(Set(Failed(t, msg))) // stay passive

  case class Success(t: Int) extends fact:
    watch: 
      case _ => IO.pure(Set(Success(t))) // stay passive
// ---------------------------------------------------------------------------------------------------------------------------------------------
...
class Observe_01[T1](
  ...
  override def Init(): Unit = Initial()

  case class Initial() extends fact:
    watch:
      case Time(t_NEW) if conditionStep.isEmpty || conditionStep.get.isInOneOfStates("Done") =>
        IO.pure(Set(Active[T1](t_NEW, t_NEW, None, isInRange01, untilStep01, untilState01, observedEventName)))
